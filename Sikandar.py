...
bot = telebot.TeleBot('7511149271:AAFabb8p0-whX7fqqFKbCVKsthr9mIsjxnA')
...
def authorize_user(message, func):
    ...
    if is_key_valid(message.text):
        ...
        bot.reply_to(message, "🚫KEY EXPIRED OR INVALID❌ GET NEW KEY 🔑 @God_x_Sikandar✅.")    
...
➡️ TO Start @God_x_Sikandar Ddos Attack  : /bgmi 
...
Buy From :- @God_x_Sikandar

Official Channel :- https://t.me/Bgmiserverhackkk
'''
...
➡️ Join Telegram :- https://t.me/Bgmiserverhackkk
...
def set_bot_commands():
    ...
    BotCommand("id", "Get User ID"),
        ...
        BotCommand("bgmi", "START God_x_Sikandar DDOS ATTACK"),
...
def create_user_keyboard():
    ...
    button1 = KeyboardButton("♻️ START God_x_Sikandar DDOS ATTACK ♻️")
...
def create_admin_keyboard():
    ...
    button8 = KeyboardButton("DELETE KEY 🔑")
...
@bot.message_handler(func=lambda message: message.text in ["♻️ START God_x_Sikandar DDOS ATTACK ♻️", "RULES ℹ️", "MY LOGS 📝", "HELP ❓", "BUY PLAN 🛒"])
def handle_user_buttons(message):
    user_id = str(message.chat.id)
    
    if user_id in allowed_user_ids:
        if message.text == "♻️ START God_x_Sikandar DDOS ATTACK ♻️":
            handle_bgmi(message)
...
@bot.message_handler(func=lambda message: message.text in ["♻️ START God_x_Sikandar DDOS ATTACK ♻️", "LOGS 📊", "ALL USERS 👥", "CLEAR LOGS 🗑️"])
